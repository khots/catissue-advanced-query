...
//	private ResultSet result =null;
	...
	public String buildQuery(IQuery query,char QueryType) throws MultipleRootsException, SQLException, RuntimeException, SQLXMLException, SqlException, DynamicExtensionsSystemException
	{
...
//        }
	...
	public static Object getObjectCopy(Object obj)
	{
		...
		public List<OutputTreeDataNode> getRootOutputTreeNodeList() {
		 ...
		 public String getSelectPart(char QueryType) throws SQLException, RuntimeException, MultipleRootsException, SQLXMLException, DynamicExtensionsSystemException
		    {
		    	...
		    	{
		    		xmlGetForQueryPart.append("$" + tableName + " in " + "db2-fn:xmlcolumn(\"" + tableName + ".XMLDATA\") ,");
		    	}
		    	...
		    	for(IExpression expressions : constraints)
		    	{
		    		...
		    		List<AssociationInterface> associationList = QueryCSMUtil.getIncomingContainmentAssociations(entity);
					if (associationList.size() != 0)
					{
						for(AssociationInterface associations : associationList)
						{
							...
							if(!(i == 1))
							{
								XPath = getAppropriatePath(entity,XPath,EntityList);
								xmlGetForQueryPart.append("$" + associations.getTargetEntity().getName() + "in" +  XPath + "/" + associations.getTargetEntity().getName());
							}
						}
	    		...
	    		for(IExpression expressions : constraints)
	    		{
	    			...
	    			for(AttributeInterface attribute : attributes)
	    			{
	    				...
	    				{
	    					xmlGetLetQueryPart.append("$" + entity.getName().toString() + ":= $" + tableName + "/"  + Path + "/" +  attribute.getName().toString() + " ,");
	    				}
	    				else
	    				{
	    				xmlGetLetQueryPart.append("$" + attribute.getName().toString() + ":= $" + tableName + "/"  + Path + "/" +  attribute.getName().toString() + " ,");
	    				}
	    			}
	    		}
			...
			private String getAppropriatePath(EntityInterface entity, String path,List<String> EntityList) throws DynamicExtensionsSystemException {
			String sourceEntity = "";
			String tableName = "";
			List<AssociationInterface> associationList = QueryCSMUtil.getIncomingContainmentAssociations(entity);
			for(AssociationInterface associations : associationList)
			{
				...
				if(EntityList.contains(sourceEntity))
				{
					try
					{
						...
						if(tableName.substring(2).equalsIgnoreCase("DE"))
						{
							break;
						}
						else
				...
				else
				{
					...
					path = getAppropriatePath(entity,path,EntityList) + "/" + path;
				}
			}
		    ...
		    private String processComparisionOperator(ICondition condition, String attributeName) {
		        ...
		        String sqlXmlCond = "$" + attributeName + " " + RelationalOperator.getSQL(operator) + " " + value ;
		        return sqlXmlCond;
		    }
			...
			for(IExpression expression : constraints)
			{
				...
				EntityInterface entity = expression.getQueryEntity().getDynamicExtensionsEntity();
				List<EntityInterface> mainRoot = QueryCSMUtil.getAllMainEntities(entity, mainEntityList);
