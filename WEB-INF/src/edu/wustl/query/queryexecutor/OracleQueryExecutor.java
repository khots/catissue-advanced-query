...
public class OracleQueryExecutor extends AbstractQueryExecutor
{
	 ...
	 * @throws DAOException 
	 * @throws SMException 
	 * @see edu.wustl.query.queryexecutor.AbstractQueryExecutor#createStatemtentAndExecuteQuery()
	 */
	protected PagenatedResultData createStatemtentAndExecuteQuery(JDBCDAO jdbcDAO)
	...
	protected String putPageNumInSQL(String sql, int startIndex, int noOfRecords)
	{
		 ...
		 */
		newSql.append(SELECT_CLAUSE + " * " + FROM_CLAUSE + " (" + SELECT_CLAUSE
				+ " qry.*, ROWNUM rn " + FROM_CLAUSE + " (" + sql + ") qry WHERE ROWNUM <= "
				+ startIndex + noOfRecords + ") WHERE rn > " + startIndex);
