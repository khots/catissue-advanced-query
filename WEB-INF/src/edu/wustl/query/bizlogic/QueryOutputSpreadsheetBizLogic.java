...
public class QueryOutputSpreadsheetBizLogic
{
		...
		if (parentNode.contains("NULL"))
		{
			...
			if (!selectedColumnMetaData.isDefinedView())
			{
				spreadSheetDataMap = createSpreadsheetData(treeNo, root, queryDetailsObj, null,
						recordsPerPage, this.selectedColumnMetaData, queryResultObjectDataBeanMap,
			...
			else
			{
				...
				spreadSheetDataMap = createSpreadsheetData(treeNo, root, queryDetailsObj, null,
						recordsPerPage, this.selectedColumnMetaData, queryResultObjectDataMap,
						hasConditionOnIdentifiedField, constraints, outputTermsColumns);
	...
	public Map processSpreadsheetForDataNode(QueryDetails queryDetailsObj,
			...
			SelectedColumnsMetadata selectedColumnMetaData, boolean hasConditionOnIdentifiedField,
			Map<Long, QueryResultObjectDataBean> queryResultObjectDataMap,
			IConstraints constraints, Map<String, IOutputTerm> outputTermsColumns)
			throws DAOException, ClassNotFoundException
	{
		...
		if (!selectedColumnMetaData.isDefinedView())
		{
			spreadSheetDatamap = createSpreadsheetData(treeNo, parentNode, queryDetailsObj,
					...
					queryResultObjectDataBeanMap, hasConditionOnIdentifiedField, constraints,
					outputTermsColumns);
			spreadSheetDatamap.put(Constants.QUERY_REASUL_OBJECT_DATA_MAP,
					queryResultObjectDataBeanMap);
		}
		else
		{
			...
			spreadSheetDatamap = createSpreadsheetData(treeNo, parentNode, queryDetailsObj,
					parentData, recordsPerPage, this.selectedColumnMetaData,
					queryResultObjectDataMap, hasConditionOnIdentifiedField, constraints,
					outputTermsColumns);
			spreadSheetDatamap.put(Constants.DEFINE_VIEW_QUERY_REASULT_OBJECT_DATA_MAP,
					queryResultObjectDataMap);
		}
			...
			DAOException
	{
		...
		spreadSheetDataMap.put(Constants.QUERY_SESSION_DATA, querySessionData);
		spreadSheetDataMap
				.put(Constants.QUERY_REASUL_OBJECT_DATA_MAP, queryResultObjectDataBeanMap);
...
//			totalFileTypeAttributes = fileTypeAtrributeIndexMetadataMap.size();
			...
			QueryDetails queryDetailsObj, String parentData)
	{
		...
		while (iterator1.hasNext())
		{
			...
			if (node == null
					|| (expressionsInTerm.size() == 1 && isContainingExpressionInTQ(node,
							expressionsInTerm)))
			{
			...
			{
				boolean isValidCardinality = isValidCardinality(temporalColumnUIBean,
						queryDetailsObj, parentData, node);
	...
	private boolean isValidCardinality(TemporalColumnUIBean temporalColumnUIBean,
			QueryDetails queryDetailsObj, String parentData, OutputTreeDataNode node)
	{
		String tableName = Constants.TEMP_OUPUT_TREE_TABLE_NAME
				+ queryDetailsObj.getSessionData().getUserId() + queryDetailsObj.getRandomNumber();
		...
		{
			Set<ICustomFormula> customFormulasInQuery = QueryUtility
					.getCustomFormulas(queryDetailsObj.getQuery());
		...
		if (parentData == null)
		{
			sqlTofindOutCardinality = "select count(*) from (select " + clickedExpIdColumn
					+ ", count(distinct " + anotherExpIdColumn + ") as card from " + tableName
					+ " group by " + clickedExpIdColumn + " having card >1) x ";
		...
		{
			sqlTofindOutCardinality = "select count(distinct " + anotherExpIdColumn + ") from "
					+ tableName + " where " + clickedExpIdColumn + "= " + parentData;
			int intCount = executeSqlToGetCount(sqlTofindOutCardinality);
			if (intCount == 1 || intCount == 0)
			{
				return true;
			...
			Set<IExpression> expressionsInTerm)
	{
		...
		Collection<AssociationInterface> associationCollection = node.getOutputEntity()
				.getDynamicExtensionsEntity().getAllAssociations();
...
//			}
			...
			Map<Long, QueryResultObjectDataBean> queryResultObjectDataBeanMap)
	{
		...
		for (Iterator<Integer> columnNameIterator = fileTypeIndexColumnNameMap.keySet().iterator(); columnNameIterator
				.hasNext();)
		{
