...
public class QueryModuleSearchQueryUtil
{
	...
	{  
		...
		try
		{
			...
			if (queryDetailsObj.getSessionData() != null)
			{
				...
				boolean hasCondOnIdentifiedField = edu.wustl.common.querysuite.security.utility.Utility
	 ...
	 * @throws QueryModuleException 
	 */
	private int setTreeData(String option, int initialValue, Vector<QueryTreeNodeData> treeData)
			throws QueryModuleException
	{
		...
		{
			List<QueryTreeNodeData> limitedRecordsList = treeData.subList(0,
					Variables.maximumTreeNodeLimit + 1);
			...
			boolean hasCondOnIdentifiedField) throws QueryModuleException
	{
		...
		{ // deepti change
			...
			Map<String, List<String>> spreadSheetDatamap = outputSpreadsheetBizLogic
					.createSpreadsheetData(Constants.TREENO_ZERO, node, QueryDetailsObj, null,
			...
			Map<String, List<String>> spreadSheetDatamap)
	{
		...
		session.setAttribute(Constants.DEFINE_VIEW_QUERY_REASULT_OBJECT_DATA_MAP,
				spreadSheetDatamap.get(Constants.DEFINE_VIEW_QUERY_REASULT_OBJECT_DATA_MAP));
	}
