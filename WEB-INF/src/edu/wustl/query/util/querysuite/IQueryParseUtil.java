...
public class IQueryParseUtil
{
	 ...
	 * @param rootNode
	 */
	public static Map<Integer, List<OutputTreeDataNode>> separateAllChildren(
			OutputTreeDataNode rootNode, IQuery query)
	{
		...
		separateChildrenNodes(query, childrenNodes, containmentsTreeDataNodesList,
				mainEntitiesTreeDataNodesList);

		//Now for each children containment of root entity, separate further till leaf nodes
		separateChildrenForEachContainment(containmentsTreeDataNodesList,
				mainEntitiesTreeDataNodesList, query);

		mainEntitiesContainmentMap.put(Integer.valueOf(rootNode.getExpressionId()),
				containmentsTreeDataNodesList);
		...
		for (int count = 0; count < mainEntitiesTreeDataNodesList.size(); count++)
		{
			...
			getAllMainEntityContainments(mainEntityContainmentList, mainTreeDataNode,
					mainEntitiesTreeDataNodesList, query);
			mainEntitiesContainmentMap.put(Integer.valueOf(mainTreeDataNode.getExpressionId()),
					mainEntityContainmentList);
		}
			...
			List<OutputTreeDataNode> mainEntitiesTreeDataNodesList)
	{
		...
		{
			addNodeToContainmentEntityList(mainEntitiesTreeDataNodesList, query,
					rootEntityContainmentList, childNode);
		}
	}
	...
	private static boolean checkIfOneOftheMainEntity(OutputTreeDataNode childNode, IQuery query)
	{
		...
		QueryableObjectInterface entity = outputEntity.getDynamicExtensionsEntity();
	 ...
	 * @param containmentsTreeDataNodesList containment entities list.
	 * @param mainEntitiesTreeDataNodesList main Entities node list.
	 * @param query query.
	 */
	private static void separateChildrenForEachContainment(
			List<OutputTreeDataNode> containmentsTreeDataNodesList,
			List<OutputTreeDataNode> mainEntitiesTreeDataNodesList, IQuery query)
	{
	 ...
	 * @param treeDataNode node to be added in the list.
	 * @param mainEntitiesTreeDataNodesList main Entities node list.
	 * @param query query.
	 * @return containment entities list.
	 */
	private static List<OutputTreeDataNode> getAllContainmentEntities(
			...
			List<OutputTreeDataNode> mainEntitiesTreeDataNodesList, IQuery query)
	{
		...
		if ((list != null) && (!list.isEmpty()))
		{
			...
			{
				addNodeToContainmentEntityList(mainEntitiesTreeDataNodesList, query,
						conatainmentEntities, outputTreeDataNode);
			}
		}
	...
	/**
	 * This method will add the outputTreeDataNode to the conatainmentEntities list if 
	 * it is the contained object else will add in mainEntitiesTreeDataNodesList.
	 * @param mainEntitiesTreeDataNodesList main Entities node list.
	 * @param query query.
	 * @param conatainmentEntities containment entities list.
	 * @param outputTreeDataNode node to be added in the list.
	 */
	private static void addNodeToContainmentEntityList(
			List<OutputTreeDataNode> mainEntitiesTreeDataNodesList, IQuery query,
			List<OutputTreeDataNode> conatainmentEntities, OutputTreeDataNode outputTreeDataNode)
	{
	 ...
	 */
	private static void getAllMainEntityContainments(
			List<OutputTreeDataNode> mainEntityContainmentList,
			...
			List<OutputTreeDataNode> mainEntitiesTreeDataNodesList, IQuery query)
	{
		...
		{
			separateChildrenForEachContainment(mainEntityContainmentList,
					mainEntitiesTreeDataNodesList, query);
		}
	}

	public static Map<OutputTreeDataNode, List<OutputTreeDataNode>> getParentChildrensForaMainNode(
			OutputTreeDataNode mainTreeDataNode)
	{
	...
	private static void getChildrenForEachMainChild(List<OutputTreeDataNode> mainChildrenList,
